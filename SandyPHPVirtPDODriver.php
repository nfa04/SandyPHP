...
final class SandyPHPVirtPDODriver_SANDBOX_SANDBOXID extends PDO {
    ...
    public function query(string $sql, ?int $fetchMode = null, mixed ...$fetchModeArgs) {
        return (checkQuery($sql, $this->connectionType, $this->config) ? parent::query($sql, $fetchMode, $fetchModeArgs) : false);
    }

    #[\Override]
    public function exec(string $statement) {
        return (checkQuery($statement, $this->connectionType, $this->config) ? parent::exec($statement) : false);
    }

    #[\Override]
    public function prepare(string $query, array $options = []) {
        return (checkQuery($query, $this->connectionType, $this->config) ? parent::prepare($query, $options) : false);
    }
